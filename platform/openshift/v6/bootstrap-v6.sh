...
[[ "$READY" == "true" ]] || {
  ...
  for d in "${NPLUSONE[@]}"; do
    ...
    echo "$d pdb=$(oc get pdb "$d" -n "$PROJECT" -o jsonpath='{.spec.minAvailable}' 2>/dev/null || echo '<missing>')"
  done
...
echo "V6 bootstrap OK: Argo CD reconciles six N+1 workloads at two replicas with PDB minAvailable=1; mock-sct-inst now uses shared PostgreSQL settlement state. PostgreSQL, Kafka and Keycloak remain single-replica dependencies."
