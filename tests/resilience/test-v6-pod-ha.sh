...
NPLUSONE=(api-gateway payment-service consumer-psp event-audit-service mock-wero mock-sct-inst)
...
echo "==> 2. Verify V6 N+1 baseline"
for d in "${NPLUSONE[@]}"; do
...
echo "==> 3. Kill one pod per N+1 component and measure recovery"
for d in "${NPLUSONE[@]}"; do
...
echo "==> 4. Record remaining single-replica dependencies"
for d in "${STATEFUL_SPOF[@]}"; do
  REPLICAS="$(oc get deployment "$d" -n "$PROJECT" -o jsonpath='{.spec.replicas}')"
  [[ "$REPLICAS" == "1" ]] || { echo "$d expected single-replica dependency baseline, got $REPLICAS"; exit 1; }
...
echo "V6 OK (pod HA current state): six N+1 components, including shared-state mock-sct-inst, survived one-pod loss and recovered to two replicas; V5 business/observability regression still passes. PostgreSQL, Kafka and Keycloak remain explicit single-replica dependencies."
