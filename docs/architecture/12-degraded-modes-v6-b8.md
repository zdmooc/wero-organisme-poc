...
| API Gateway unavailable | Public payment API is unavailable | Public creation cannot reach backend through the Route | Public reads are unavailable | No payment, rail or ledger row is created by the blocked public request | Retry the untouched authorized client intent after gateway recovery | Not required because the failed request never entered the payment system | Two CRC runs: **16 s**, then **11 s** | V3B backend isolation + no backend side effect during outage + idempotent retry; final payment settled once |
...
The public Route returned either HTTP `503` or curl `000` for the read request depending on timing during total gateway unavailability, and `503` for create in the observed runs. Both outcomes represent the same expected degraded state: the public dataplane was unavailable and the blocked request produced no backend side effect.
...
Both outage experiments temporarily suspended Argo CD self-heal only to inject the failure. Desired replicas were restored and self-heal re-enabled before each run passed.

## Completion status

B8 is complete on CRC. The full Phase B degraded-mode coverage is now composed of:
...
The next and final CRC gate for V6 is the full V4/V5/V6 regression. V6 CRC must not be declared complete until that regression passes.
